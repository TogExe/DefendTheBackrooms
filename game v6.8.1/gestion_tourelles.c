...
void turret(Tower* towers, int tower_count, Enemy* enemies, int enemy_count, Mix_Chunk *tir_son, int tile_size) {	
    for (int i = 0; i < tower_count; i++) {
        if (towers[i].active_shoot > 0&&rand()%2) {
        ...
        for (int j = 0; j < enemy_count; j++) {
            ...
            if (distance_squared <= towers[i].range * towers[i].range&&enemies[j].y >1) {
            	printf("enemy_cy");
...
void upgrade(Tower* towers, int tower_count, int mouse_x, int mouse_y, int tile_size, int*money) {
    for (int i = 0; i < tower_count; i++) {
        ...
        if (mouse_x >= rect.x && mouse_x <= rect.x + tile_size &&
            ...
            towers[i].range +=1;
            towers[i].level += 1;
            if (2<towers[i].level&&towers[i].level<8){towers[i].speed_damage -= 1;}
...
void upgrade_s(Tower* towers, int tower_count, int x, int y) {
    for (int i = 0; i < tower_count; i++) {
        int tx = towers[i].x;
        int ty = towers[i].y;
        ...
        if (x==tx&&y==ty) {
            ...
            towers[i].range +=1;
            towers[i].level += 1;
            if (2<towers[i].level&&towers[i].level<8){
            	towers[i].speed_damage -= 1;
            }
            printf("Tour %d niveau %d\n", i, towers[i].level);
            return;
        }
