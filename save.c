...
void sauvegarder_partie(const char *filename, int seed, int wave, Tower *towers, int tower_count) {
    ...
    for (int i = 0; i < tower_count; i++) {
        fprintf(f, "    {\"x\": %d, \"y\": %d}%s\n", towers[i].x, towers[i].y, i == tower_count - 1 ? "" : ",");
    }
    fprintf(f, "  ]\n");
    fprintf(f, "}\n");
...
int charger_partie(const char *filename, int *seed, int *wave, Tower *towers, int *tower_count) {
    ...
    sscanf(buffer, " { \"seed\": %d , \"wave\": %d", seed, wave);

    // Recherche manuelle des tourelles
    *tower_count = 0;
    ...
    if (ptr) {
        ...
        while (*ptr && *ptr != ']') {
            int x, y;
            if (sscanf(ptr, " { \"x\": %d , \"y\": %d }", &x, &y) == 2) {
                towers[*tower_count] = (Tower){ .x = x, .y = y, .damage = 1, .range = 48, .speed_damage = 30, .cooldown = 0 };
                (*tower_count)++;
            }
            ptr = strchr(ptr, '}');
            if (!ptr) break;
