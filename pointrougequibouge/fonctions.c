...
void waiting(SDL_Renderer* renderer, SDL_Window* window, TTF_Font* font, int* start, int* vie, int* argent) {
    ...
    SDL_Rect widget_rect = {
        window_width - 200,           // Position X (droite)
        window_height / 2 - 75,       // Centré verticalement
        150,
        150
    };
    ...
    while (!*start) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN) {
                ...
                // Clic dans la zone "START"
                if (mx >= widget_rect.x + 25 && mx <= widget_rect.x + 125 &&
                    my >= widget_rect.y + 90 && my <= widget_rect.y + 120) {
                    *start = 1;
                }
            }
        }
