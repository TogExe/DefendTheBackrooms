...
void initialize(char *** grid1, char *** map1,Tower ** towers, Enemy ** enemies,
	...
	*t_count =0; 
	*click_count =0; 
	*argent=30+(60-(rand()%*side)); //Smaller maps cause more difficulty so more money
	*map1=map;
	*grid1=grid;
...
int main() {
    ...
    Mix_Music *musique2 = Mix_LoadMUS("assets/play2.mp3");
    ...
    SDL_Texture *fleur = charge_texture(renderer, "assets/fleur_rose.png");
    SDL_Texture *rock = charge_texture(renderer, "assets/rock.png");
    SDL_Texture *path = charge_texture(renderer, "assets/path.png");
    if (!icon_texture || !icon_texture1 ||!bush ||!rock||!fleur||!path) return -1;
	 ...
	 while (c.running) {
    	if(c.menu==START){
			...
			if(c.new){
				...
				printf("\nthe line 258 worked\n");
				c.menu = PLAY;
				sauvegarder_partie(SAVE,seed,wave,vie,argent,towers,tower_count);
			}else{
				charger_seed(SAVE,&seed);
				initialize(&grid, &map, &towers, &enemies,
										 &click_count, &argent, &current_enemy_count, 
										 &seed, &side, &tile_size,&tower_count);
				tower_count =0;
				charger_partie(SAVE,&seed,&wave,&vie,&argent,&towers,&tower_count);
				c.menu =PLAY;
			}
    		interact_gui(&gui);
    		update_gui(&gui,&c);
	        ...
	        /*while (SDL_PollEvent(&e)) {
                            ...
                            if (e.type == SDL_MOUSEBUTTONDOWN && !c.as) {
                              ...
                              if (click_count < MAX_CLICKS && draw_click_zone(renderer, x, y, side, grid, tile_size, & final_x, & final_y, click_positions, click_count, argent)) {
                                ...
                                towers[tower_count++] = (Tower) {
                                  .x = final_x, .y = final_y, .damage = 1, .range = 3 * tile_size, .speed_damage = 11, .cooldown = 0
                                };
                                num_towers = tower_count;
                              }
                            }
                ...
                if (e.type == SDL_MOUSEBUTTONDOWN && !c.as) {
                              ...
                              SDL_GetMouseState( & x, & y);
                              if (click_count < MAX_CLICKS && draw_click_zone(renderer, x+tile_size/2, y+tile_size/2, side, grid, tile_size, & final_x, & final_y, click_positions, click_count, argent-2)) {
                                click_positions[click_count][0] = final_x;
                                click_positions[click_count][1] = final_y;
