...
void initialize(char *** grid1, char *** map1,Tower ** towers, Enemy ** enemies,
	...
	char ** grid = *grid1;
	char ** map  = *map1;
	// === PROCEDURE usable to restart or start the game ===
	//printf("\n\nTrying to free\n\n");
	// We free everything
	if (grid!=NULL) {
		//printf("help\n");
	...
	//char **grid;
	//char **map;
	// === Now we generate the new or old map depending on the seed
	...
	*tile_size = 512/ *side;
	...
	//printf("\n WORLD HAS BEEN GENERATED :D with a size of %d\n",*side);
	//printf("WTF WHERE IS THE SEGMENTATION FAULT\n");
	/*
	for (int i=0; i< *side;i++){
		printf(" %d",i);
...
int main() {
    ...
    if (!icon_texture || !icon_texture1 ||!bush ||!rock||!fleur) return 1;
