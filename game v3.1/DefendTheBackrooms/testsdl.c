...
void initialize(Tower * towers, Enemy * enemies, int * click_count, int * argent, int * current_enemy_count, int * seed, int * side, int * tile_size){
	if(towers !=NULL){
		free(towers);
	}
	if(enemies!=NULL){
...
int main() {
    ...
    SDL_Window *window = SDL_CreateWindow("Map 2D Aleatoire", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 500,500, 0);
