...
void draw_hud(SDL_Renderer *renderer, TTF_Font *font, int vie, int argent,int wave) {
    ...
    SDL_Rect background = {0, 0, 150, 90}; // Rectangle noir semi-transparent en haut à gauche

    // Dessiner le fond
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 170); // Noir semi-transparent
	...
	// WAVE
    snprintf(buffer, sizeof(buffer), "Vague : %d", wave-1);
    SDL_Surface *surface_arg1 = TTF_RenderText_Blended(font, buffer, textColor);
    SDL_Texture *texture_arg1 = SDL_CreateTextureFromSurface(renderer, surface_arg);
    SDL_Rect dest_arg1 = {10, 57, surface_arg1->w, surface_arg1->h};
    SDL_FreeSurface(surface_arg1);
    SDL_RenderCopy(renderer, texture_arg1, NULL, &dest_arg1);
    SDL_DestroyTexture(texture_arg1);    
